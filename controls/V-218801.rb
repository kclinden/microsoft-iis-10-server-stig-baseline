...
control 'V-218801' do
  title "Java software installed on a production IIS 10.0 web server must be
...
easier to analyze, and is less complicated to navigate through the hosted
...
mobile code downloaded and executed on individual workstations.
    ...
    Source code for a Java program is often stored in files with either .java
or .jpp file extensions. From the .java and .jpp files the Java compiler
...
and permissions to resources on the server.
  ...
  "
  desc  'fix', "Remove all files from the web server with both .java and .jpp
extensions."
  impact 0.5
  tag severity: 'medium'
