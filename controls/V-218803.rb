...
control 'V-218803' do
  title "The IIS 10.0 web server must separate the hosted applications from
...
application use is minimized.
  ...
  desc  'check', "
    ...
    If the IIS 10.0 web server does not host any applications, this is Not
Applicable.

    If the IIS 10.0 web server is hosting Exchange, this is Not Applicable.

    If the IIS 10.0 web server hosts applications, review the application's
...
with the same functions and authentication methods as the web server management.
    ...
    If the IIS 10.0 web server management and the application's management
functionality is not separated, this is a finding.
  "
  desc  'fix', "Develop a method to manage the hosted applications, either by
moving its management functions off of the IIS 10.0 web server or by accessing
