...
the hosted application since HTTP/HTTPS is a stateless protocol.
    ...
    Using URI will embed the session ID as a query string in the Uniform
Resource Identifier (URI) request and then the URI is redirected to the
originally requested URL. The changed URI request is used for the duration of
the session, so no cookie is necessary.

    By requiring expired session IDs to be regenerated while using URI,
potential attackers have less time to capture a cookie and gain access to the
Web server content.
...
from the drop-down list.
    If the \"Cookie Settings\" \"Mode\" is not set to \"Use Cookies\", this is
a finding.
...
specific system IPs, this is Not Applicable.
  ...
  desc  'fix', "
    ...
    Under \"Cookie Settings\", select \"Use Cookies” from the \"Mode\"
drop-down list.
