...
control 'V-218798' do
  title "The IIS 10.0 web server must have Multipurpose Internet Mail
...
unintentional manner.
    ...
    A MIME tells the web server the type of program, various file types, and
extensions and what external utilities or programs are needed to execute the
file type.

    A shell is a program that serves as the basic interface between the user
and the operating system to ensure hosted application users do not have access
to these programs. Shell programs may execute shell escapes and can perform
...
shell program extensions, to include at a minimum, the following extensions:
  ...
  tag cci: ['SV-109235', 'V-100131', 'CCI-000381']
  tag nist: ['CM-7 a']
