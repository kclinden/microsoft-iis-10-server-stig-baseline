...
specific, etc.) must be deleted from the IIS 10.0 server."
  desc  "Accounts used for web server features such as documentation, sample
code, example applications, tutorials, utilities, and services created when the
feature is not installed, become an exploitable threat to a web server.
...
and must be deleted when these features are uninstalled.
  ...
  desc  'check', "
    ...
    Access “Apps” menu. Under “Administrative Tools”, select “Computer
Management”.
    ...
    If any local accounts are present and were created by features which have
been uninstalled or are not used, this is a finding.
  "
  desc  'fix', "
    Access the IIS 10.0 web server.
