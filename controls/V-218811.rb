...
control 'V-218811' do
  title "Remote access to the IIS 10.0 web server must follow access policy or
...
communication channel must be used."
  ...
  desc  'check', "
    ...
    If web administration is performed remotely, the following checks will
apply:

    If administration of the server is performed remotely, it will only be
performed securely by system administrators.

    If website administration or web application administration has been
...
authorized personnel.
    ...
    All remote sessions will be encrypted and utilize FIPS 140-2-approved
protocols.

    FIPS 140-2-approved TLS versions include TLS V1.2 or greater.

    Review with site management how remote administration is configured on the
website, if applicable.

    If remote management meets the criteria listed above, this is not a finding.

    If remote management is utilized and does not meet the criteria listed
above, this is a finding.
  "
  desc  'fix', "Ensure the web server administration is only performed over a
secure path."
  impact 0.7
  tag severity: 'high'
