...
control 'V-218815' do
  title "The IIS 10.0 web server must use a logging mechanism configured to
allocate log record storage capacity large enough to accommodate the logging
requirements of the IIS 10.0 web server."
  desc  "To ensure the logging mechanism used by the web server has sufficient
storage capacity in which to write the logs, the logging mechanism must be able
to allocate log record storage capacity.
...
regular basis, this is a finding.
  ...
  desc  'fix', "
    ...
    If necessary, in the \"Logging\" configuration box, re-designate a log path
